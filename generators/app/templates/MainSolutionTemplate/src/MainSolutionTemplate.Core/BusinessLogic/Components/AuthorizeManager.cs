...
namespace MainSolutionTemplate.Core.BusinessLogic.Components
{
    public class AuthorizeManager : IAuthorizeManager
    {
        ...
        public bool IsAuthorizedActivity(Activity[] activities, string roleName)
        {
            ...
            Role rolesByName = _systemManager.GetRoleByName(roleName);
            if (rolesByName == null)
            {
                _log.Warn(
                    "AuthorizeManager:IsAuthorizedActivity Claim has a role called {0}. Currently we do not support that role");
                return false;
            }
